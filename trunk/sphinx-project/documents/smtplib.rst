...
.. code-block:: python
    ...
    try:
        ...
        # identify ourselves, prompting server for supported features标识自己身份,确定服务器支持的特征
        server.ehlo()

        # If we can encrypt this session, do it如果我们可以加密这个会话,就进行
        if server.has_extn('STARTTLS'):
             server.starttls()
             server.ehlo() # re-identify ourselves over TLS connection再次标识身份通过TLS连接
